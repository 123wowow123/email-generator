...
test('Should send databound email template async', async () => {
  ...
  const mailClient = new MailClient();
  const sendSignupData = await mailClient.sendSignup(defaultConfig.mail.sendSignupEmailMock.to, data);
  expect(sendSignupData.response.startsWith('250')).toEqual(true);

  const template = new Template({ connectionString: defaultConfig.azureStorage.azureConnectionString });
  const templateData = await template.getBoundTemplateByUri(defaultConfig.mail.sendSignupEmail.signupTemplateUri, data);
