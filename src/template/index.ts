...
export class Template {
    ...
    constructor(config: ITemplateOptions) {
        ...
        this.blobSvc = azure.createBlobService(this.config.connectionString)
    ...
    public async getTemplateByUri(blobPath: string): Promise<IEmailTemplate> {
        const blobPromise = new Promise<IEmailTemplate>((resolve, reject) => {
            this.blobSvc.getBlobToText(this.container, blobPath,
                (err: any, blobData: string, blob: BlobService.BlobResult, response: ServiceResponse) => {
    ...
    public async getBoundTemplateByUri(blobPath: string, data: any): Promise<IEmailTemplate> {
